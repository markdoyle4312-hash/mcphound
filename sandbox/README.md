...
4. Containers are destroyed after each scan; scan artifacts persist only via the mcphound results JSON.
...
# mcphound will publish a scan driver:
# uv run mcphound sandbox-run <server-ref> --output results.json
```
