...
def upgrade() -> None:
    """Upgrade schema."""
    op.alter_column(
        'hashes', 'observed_at', server_default=sa.text('clock_timestamp()')
    )
    op.alter_column(
        'scans', 'scanned_at', server_default=sa.text('clock_timestamp()')
    )
    op.alter_column(
        'server_scores', 'computed_at', server_default=sa.text('clock_timestamp()')
    )
