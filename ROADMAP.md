...
- [ ] **W15** — FastAPI read-only API (`/v1/servers/{id}`, `/v1/check?name=`), rate-limited, free tier; embeddable badge for server authors. *Done: `GET /v1/servers/{slug}`, `GET /v1/check?name=`, and `GET /v1/badge/{slug}.svg` are implemented with slowapi rate limiting (60/min, 60/min, 300/min) and documented in `docs/api.md`. Not yet deployed publicly — runs locally only. 2026-08-30.*
