...
- [ ] **W3** — Rule engine v1: YAML rule loader, findings dataclass, JSON output; first 4 static rules (hardcoded secrets in config, dangerous launch commands/curl-pipe-shell, over-broad filesystem/shell permissions, pinned-version check). *Done: each rule has the 4 artifacts (YAML + malicious fixture + benign fixture + test) per the `rule-authoring` skill.*
- [ ] **W4** — Next 4 static rules: tool-description injection markers (hidden comments, zero-width chars, exfiltration imperatives), typosquat distance vs a bundled name list, package provenance checks (postinstall scripts, missing source repo, package age via registry API). *Done: 8 rules passing; `--json` deterministic.*
- [ ] **W5** — SARIF output + `--fail-on` exit codes + README with install/quickstart; publish to PyPI; tag v0.1. *Done: `uvx mcpvet@0.1.0 scan <config>` works on a stranger's machine.*
