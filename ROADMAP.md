...
- [ ] **W6** — Launch post: Show HN, r/mcp, r/cybersecurity, X/LinkedIn; dogfood on your own `.mcp.json`. *Done: 100 stars target; collect first 5 issues.* Dogfood run 2026-08-29: `mcphound scan .mcp.json` on this repo — clean, no findings, verified against the published PyPI package from a fully clean environment.
...
- [ ] **W14** — **Static site generator** (not a web app): per-server score pages from nightly snapshots, leaderboard, typosquat cluster pages. Deploy cheap (Vercel/Cloudflare Pages). *Done: pages live for all scanned servers.* *Partial: the Next.js site is built — leaderboard, paginated listing, per-server pages, typosquat watchlist, `registry-export`/`typosquat-clusters.json` data pipeline, `site-build` CI smoke test — but nothing is deployed publicly yet; the Vercel project/secrets and the nightly poll→scan→export→deploy job (`nightly.yml`, still `if: false`) are still open. 2026-08-30.*
...
- [ ] **W16** — **Manual spot-check sprint**: hand-verify scores on ~50 servers (credibility gate — a bad viral "it flagged a legit server" take costs more than delay). FP feedback button live. *Not started: the human spot-check itself (0/50) hasn't happened. Worth noting given the "credibility gate" framing above — until the 2026-08-30 CI fix, every prior week's "Done"/"verified locally" claim in this roadmap and CHANGELOG.md had never actually been checked by CI, only asserted. CI now runs for real, which is a precondition for this gate meaning anything, but doesn't substitute for it — the actual score spot-check is still fully open.*
